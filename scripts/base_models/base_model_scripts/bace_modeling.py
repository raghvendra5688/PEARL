"""
BACE Base-Model-Embedding Modeling Pipeline
...
BASE_MODEL_ROOT = str(REPO_ROOT / "data" / "base_models" / "bace_datasets")
OUTPUT_ROOT = str(REPO_ROOT / "results" / "base_models" / "bace_dataset")
...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "bace_base_model_ml.log"),
    level=logging.INFO,
...
logging.info(
    f"BACE class distribution | 0: {n_neg}, 1: {n_pos}, scale_pos_weight={scale_pos_weight:.3f}"
)
...
for emb_name in EMBEDDING_MODELS:
    ...
    logging.info(f"Completed BACE modeling for embedding: {emb_name}")

logging.info("All BACE base-model embedding experiments completed successfully.")
# nohup python bace_modeling.py > bace_base_models_nohup.out 2>&1 &
