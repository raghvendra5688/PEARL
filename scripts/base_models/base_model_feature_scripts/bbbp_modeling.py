"""
BBBP Embedding + RDKit Feature Modeling Pipeline
...
DATA_ROOT = str(REPO_ROOT / "data" / "base_models_features" / "bbbp_datasets")
OUTPUT_ROOT = str(REPO_ROOT / "results" / "base_models_features" / "bbbp_dataset")
...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "bbbp_base_model_feature_ml.log"),
    level=logging.INFO,
...
# Main execution loop for BBBP task and each embedding
...
for EMB_NAME in EMBED_COLS:
    ...
    logging.info(f"Completed BBBP | {EMB_NAME}")

logging.info("BBBP base model with features modeling completed successfully.")
# nohup python bbbp_modeling.py > bbbp_base_models_features_nohup.out 2>&1 &