"""
BACE Embedding + RDKit Feature Modeling Pipeline
...
DATA_ROOT = str(REPO_ROOT / "data" / "base_models_features" / "bace_datasets")
OUTPUT_ROOT = str(REPO_ROOT / "results" / "base_models_features" / "bace_dataset")
...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, "bace_base_model_feature_ml.log"),
    level=logging.INFO,
...
def build_feature_matrix(df, emb_col):
...
# main execution loop
logging.info("========== BACE TASK: Class ==========")

for EMB_NAME in EMBED_COLS:

    logging.info(f"---- Embedding: {EMB_NAME} ----")

    X_train, y_train = build_feature_matrix(train_df, EMB_NAME)
    X_val,   y_val   = build_feature_matrix(val_df,   EMB_NAME)
    X_test,  y_test  = build_feature_matrix(test_df,  EMB_NAME)
    ...
    def train_eval(name, model):
        ...
        logging.info(f"[BACE | {EMB_NAME}] {name} metrics: {metrics}")
        return y_prob
    ...
    logging.info(f"Completed BACE | {EMB_NAME}")

logging.info("BACE base model with features modeling completed successfully.")
# nohup python bace_modeling.py > bace_base_models_features_nohup.out 2>&1 &
