"""
Uni-Mol RAG-Augmented Modelling Pipeline — ClinTox

Binary classification (primary target: FDA_APPROVED) using:
    [Uni-Mol 2560-dim embedding] + [PC features] + [ZINC-250k RAG features]

Input:
    data/unimol_embeddings/clintox_Embeddings/clintox_{split}_embed.csv
    data/finetuned_pc_embeddings/clintox_Embeddings/clintox_{split}_features.csv
    data/rag_features_unimol/clintox/{col_name}_{split}_rag.csv

Output:
    results/rag_unimol/clintox/{col_name}/ — metrics, models, ROC/PR curves
"""
...
class Config:
    UNIMOL_EMBED_ROOT = EXTRAS_ROOT / "unimol_embeddings" / "clintox_Embeddings"
    PC_EMBED_ROOT     = EXTRAS_ROOT / "PC_FT_All_Embeddings" / "clintox_Embeddings"
    RAG_ROOT          = REPO_ROOT / "data" / "rag_features_unimol" / "clintox"
    OUTPUT_ROOT       = REPO_ROOT / "results" / "rag_unimol" / "clintox"
    LOG_DIR           = OUTPUT_ROOT / "logs"

    LABEL_COL     = "FDA_APPROVED"    # primary ClinTox target (same as existing pipeline)
...
def setup_logging(config: Config) -> None:
    ...
    fh  = logging.FileHandler(config.LOG_DIR / "rag_unimol_clintox.log", mode="a")
    fh.setFormatter(fmt)
...
def load_split(config: Config, split: str) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
...
def load_rag_features(config: Config, col_name: str, split: str) -> Optional[pd.DataFrame]:
    ...
    if not path.exists():
        logging.warning(f"RAG features not found: {path}"); return None
...
def build_feature_matrix(
    ...
    if pc_df is not None:
        pc_cols_to_add = [
            ...
            if c not in [config.SMILES_COL, config.LABEL_COL, "CT_TOX"] + _OTHER_EMB_COLS
        ]
        merged = merged.merge(
            pc_df[[config.SMILES_COL] + pc_cols_to_add], on=config.SMILES_COL, how="inner"
        )
    rag_drop = [c for c in df_rag.columns if c in [config.LABEL_COL, "CT_TOX"]]
    merged = merged.merge(df_rag.drop(columns=rag_drop, errors="ignore"), on=config.SMILES_COL, how="inner")

    emb = parse_embedding_col(merged[emb_col])
    rag_cols  = [c for c in df_rag.columns if c not in [config.SMILES_COL, config.LABEL_COL, "CT_TOX"]]
    skip_cols = [config.SMILES_COL, config.LABEL_COL, "CT_TOX"] + config.EMBED_COLS + _OTHER_EMB_COLS
...
def _objective(trial, model_type, X, y, spw, config):
    ...
    if model_type == "xgb":
        ...
        m = lgb.LGBMClassifier(class_weight="balanced", random_state=config.RANDOM_SEED,
    ...
    scores = cross_val_score(m, X, y, scoring=make_scorer(matthews_corrcoef), cv=cv, n_jobs=1)
...
def run_optimisation(model_type, X, y, spw, config):
    ...
    study.optimize(lambda t: _objective(t, model_type, X, y, spw, config),
                   n_trials=config.OPTUNA_TRIALS)
    logging.info(f"  {model_type} best MCC={study.best_value:.4f}  params={study.best_params}")
...
def run_for_embedding(emb_col, train_u, train_pc, val_u, val_pc, test_u, test_pc, config):
    ...
    if rag_train is None or rag_test is None:
        logging.warning(f"  RAG features missing for {col_name}, skipping."); return

    try:
        X_tr, y_tr = build_feature_matrix(train_u, train_pc, rag_train, emb_col, config)
        X_va, y_va = build_feature_matrix(val_u,   val_pc,   rag_val,   emb_col, config)
        X_te, y_te = build_feature_matrix(test_u,  test_pc,  rag_test,  emb_col, config)
    except Exception as e:
        logging.error(f"  Feature matrix failed for {col_name}: {e}"); return
    ...
    preds = {}
    preds["XGBoost"] = train_and_evaluate("XGBoost",
        xgb.XGBClassifier(objective="binary:logistic", eval_metric="logloss",
                           scale_pos_weight=spw, random_state=config.RANDOM_SEED,
                           tree_method="hist", n_jobs=config.N_JOBS, **best["XGBoost"]),
        X_tr, y_tr, X_te, y_te, out_dirs, col_name, config)
    preds["LightGBM"] = train_and_evaluate("LightGBM",
        lgb.LGBMClassifier(class_weight="balanced", random_state=config.RANDOM_SEED,
                            n_jobs=config.N_JOBS, verbosity=-1, **best["LightGBM"]),
        X_tr, y_tr, X_te, y_te, out_dirs, col_name, config)
    preds["CatBoost"] = train_and_evaluate("CatBoost",
        cb.CatBoostClassifier(auto_class_weights="Balanced", loss_function="Logloss",
                               random_seed=config.RANDOM_SEED, verbose=0,
...
def main():
    ...
    logging.info("=" * 70)
    logging.info("Uni-Mol RAG-Augmented ClinTox Modelling (UniMol emb + PC + ZINC RAG)")
    logging.info(f"N_JOBS={config.N_JOBS}  OPTUNA_TRIALS={config.OPTUNA_TRIALS}")
    logging.info("=" * 70)

    train_u, train_pc = load_split(config, "train")
    val_u,   val_pc   = load_split(config, "eval")
    test_u,  test_pc  = load_split(config, "test")

    for emb_col in config.EMBED_COLS:
        try:
            run_for_embedding(emb_col, train_u, train_pc, val_u, val_pc, test_u, test_pc, config)
        except Exception as e:
            logging.error(f"Failed for {emb_col}: {e}", exc_info=True)

    logging.info("\nClinTox Uni-Mol RAG modelling complete.")
