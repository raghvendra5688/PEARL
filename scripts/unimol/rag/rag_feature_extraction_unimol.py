"""
Uni-Mol RAG Feature Extraction for EffiChem-2.0

For a given dataset, queries each of the 2 task-specific Uni-Mol ZINC-250k
FAISS indices (UniMol_FL, UniMol_WL) with every molecule in the train / eval /
test splits and computes chemical-space context features from the retrieved
ZINC-250k neighbors.

Features computed per model (69 total):
...
Query embeddings are loaded from:
    data/unimol_embeddings/{Dataset}_Embeddings/{prefix}_{split}_embed.csv

PCA models are saved to EffiChem_Extras/rag_pca_unimol/{dataset}/{col_name}_pca.pkl
and must be fit on train before calling eval/test — the script processes splits
in order train → eval → test automatically.

Usage:
    python "Uni-Mol/RAG Pipeline/rag_feature_extraction_unimol.py" --dataset bace

Input:
    data/unimol_embeddings/{Dataset}_Embeddings/{prefix}_{split}_embed.csv
    EffiChem_Extras/rag_indices_unimol/{dataset}/{col_name}.index
    EffiChem_Extras/rag_indices_unimol/{dataset}/meta.pkl

Output:
    data/rag_features_unimol/{dataset}/{col_name}_{split}_rag.csv
    EffiChem_Extras/rag_pca_unimol/{dataset}/{col_name}_pca.pkl
"""
...
MODEL_REGISTRY = [
    {"col_name": "UniMol_FL", "csv_col": "UniMol_FL_embeddings"},
...
def setup_logging(dataset: str) -> None:
    ...
    logging.basicConfig(
        filename=str(LOG_DIR / f"rag_feature_extraction_unimol_{dataset}.log"),
        level=logging.INFO,
...
def safe_parse_embedding(s: str) -> Optional[np.ndarray]:
    try:
        ...
        try:
            ...
            arr = np.array(
                [float(x) for x in s_clean.split(",") if x.strip()],
                dtype=np.float32,
            )
...
def load_index_on_gpu(index_path: Path, gpu_id: int = 0) -> faiss.Index:
    ...
    logging.info(
        f"  Index loaded on GPU {gpu_id}: "
        f"{gpu_index.ntotal} vectors, d={cpu_index.d}"
    )
    return gpu_index
...
def compute_tanimoto_features(
    ...
    if qfp is None:
        return {
            "tanimoto_k1": 0.0,
            "tanimoto_k5_mean": 0.0,
            "scaffold_match_frac_k10": 0.0,
        }
    ...
    scaffold_matches = sum(
        1 for smi in neighbor_smiles
        if _murcko_scaffold(smi) == q_scaffold and q_scaffold != ""
    )
...
def compute_rag_features(
    ...
    if pca is None:
        ...
        logging.info(
            f"  PCA fitted on {centroid_matrix.shape[0]} train centroids "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract ZINC-250k Uni-Mol RAG features."
    )
    ...
    logging.info("=" * 60)
    logging.info(
        f"Uni-Mol RAG Feature Extraction | dataset={dataset} | GPU={args.gpu_id}"
    )
    logging.info("=" * 60)
    ...
    if not meta_path.exists():
        raise FileNotFoundError(
            f"meta.pkl not found: {meta_path}. Run build_zinc_index_unimol.py first."
        )
    ...
    for model_info in MODEL_REGISTRY:
        ...
        if not index_path.exists():
            logging.warning(
                f"Index not found, skipping: {index_path}. "
                "Run build_zinc_index_unimol.py first."
            )
            continue
        ...
        for split in SPLITS_ORDER:
            ...
            if csv_col not in df.columns:
                logging.warning(
                    f"  Column '{csv_col}' not in {embed_csv.name}, skipping."
                )
                continue
            ...
            out_df  = pd.concat(
                [df_valid[id_cols].reset_index(drop=True), features_df], axis=1
            )
    ...
    logging.info("\nUni-Mol RAG feature extraction complete.")
