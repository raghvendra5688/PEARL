"""
Uni-Mol RAG-Augmented Modelling Pipeline — BBBP

Binary classification (target: p_np) using:
    [Uni-Mol 2560-dim embedding] + [PC features] + [ZINC-250k RAG features]

Input:
    data/unimol_embeddings/BBBP_Embeddings/bbbp_{split}_embed.csv
    data/finetuned_pc_embeddings/BBBP_Embeddings/bbbp_{split}_features.csv   (PC features)
    data/rag_features_unimol/bbbp/{col_name}_{split}_rag.csv

Output:
    results/rag_unimol/bbbp/{col_name}/ — metrics, models, ROC/PR curves
"""
...
class Config:
    UNIMOL_EMBED_ROOT = EXTRAS_ROOT / "unimol_embeddings" / "BBBP_Embeddings"
    PC_EMBED_ROOT     = EXTRAS_ROOT / "PC_FT_All_Embeddings" / "BBBP_Embeddings"
    RAG_ROOT          = REPO_ROOT / "data" / "rag_features_unimol" / "bbbp"
    OUTPUT_ROOT       = REPO_ROOT / "results" / "rag_unimol" / "bbbp"
...
def setup_logging(config: Config) -> None:
    ...
    fh  = logging.FileHandler(config.LOG_DIR / "rag_unimol_bbbp.log", mode="a")
    fh.setFormatter(fmt)
...
def train_and_evaluate(name, model, X_tr, y_tr, X_te, y_te, out_dirs, tag, config):
    ...
    logging.info(f"  [BBBP|{tag}] {name}: {metrics}")
    ...
    with _plot_ctx():
        ...
        for name, prob in preds.items():
            ...
            axes[1].plot(rec, prec, label=f"{name} (AP={average_precision_score(y_te,prob):.3f})",
                         color=config.MODEL_COLORS[name], linewidth=2)
        axes[0].plot([0,1],[0,1],"k--"); axes[0].set_title(f"BBBP ROC — {tag}")
        axes[0].set_xlabel("FPR"); axes[0].set_ylabel("TPR"); axes[0].legend()
        axes[1].set_title(f"BBBP PR — {tag}")
...
def main():
    ...
    logging.info("=" * 70)
    logging.info("Uni-Mol RAG-Augmented BBBP Modelling (UniMol emb + PC + ZINC RAG)")
    ...
    logging.info("\nBBBP Uni-Mol RAG modelling complete.")
