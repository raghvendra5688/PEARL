"""
Uni-Mol LoRA Finetuning — HERG | Focal Loss

Mirrors finetune_herg_fl.py from lora-finetuning-scripts/ but uses
...
Best model saved to:
  EffiChem_Extras/focal_HERG/dptech__Uni__Mol_LoRA_Finetuned/
"""
...
SAVE_DIR    = EXTRAS_ROOT / "focal_loss_HERG" / "dptech__Uni__Mol_LoRA_Finetuned"
...
logging.basicConfig(
    filename=str(LOG_DIR / "unimol_herg_fl.log"),
    level=logging.INFO,
...
# ── Sweep run ──────────────────────────────────────────────────────────────────
def run_training():
    run    = wandb.init(project="HERG_UniMol_FL")
    config = run.config
...
if __name__ == "__main__":
    logging.info("=" * 60)
    logging.info("Uni-Mol LoRA Finetuning | HERG | Focal Loss")
    logging.info("Additional modalities: 3D conformer (ETKDGv3) + Morgan ECFP4")
    logging.info("=" * 60)

    sweep_id = wandb.sweep(sweep_config, project="HERG_UniMol_FL")
