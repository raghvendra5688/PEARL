"""
Uni-Mol LoRA Finetuning — DILI | Weighted Loss

Mirrors finetune_dili_wl.py from lora-finetuning-scripts/ but uses
...
Best model saved to:
  EffiChem_Extras/weighted_DILI/dptech__Uni__Mol_LoRA_Finetuned/
"""
...
SAVE_DIR    = EXTRAS_ROOT / "weighted_loss_DILI" / "dptech__Uni__Mol_LoRA_Finetuned"
...
logging.basicConfig(
    filename=str(LOG_DIR / "unimol_dili_wl.log"),
    level=logging.INFO,
...
# ── Sweep run ──────────────────────────────────────────────────────────────────
def run_training():
    run    = wandb.init(project="DILI_UniMol_WL")
    config = run.config
...
if __name__ == "__main__":
    logging.info("=" * 60)
    logging.info("Uni-Mol LoRA Finetuning | DILI | Weighted Loss")
    logging.info("Additional modalities: 3D conformer (ETKDGv3) + Morgan ECFP4")
    logging.info("=" * 60)

    sweep_id = wandb.sweep(sweep_config, project="DILI_UniMol_WL")
