"""
Uni-Mol LoRA Finetuning — FLAVOR | Focal Loss

Mirrors finetune_flavor_fl.py from lora-finetuning-scripts/ but uses
...
Best model saved to:
  EffiChem_Extras/focal_flavor/dptech__Uni__Mol_LoRA_Finetuned/
...
SAVE_DIR    = EXTRAS_ROOT / "focal_loss_flavor" / "dptech__Uni__Mol_LoRA_Finetuned"
...
logging.basicConfig(
    filename=str(LOG_DIR / "unimol_flavor_fl.log"),
    level=logging.INFO,
...
# ── Sweep run ──────────────────────────────────────────────────────────────────
def run_training():
    run    = wandb.init(project="FLAVOR_UniMol_FL")
    config = run.config
...
if __name__ == "__main__":
    logging.info("=" * 60)
    logging.info("Uni-Mol LoRA Finetuning | FLAVOR | Focal Loss")
    ...
    sweep_id = wandb.sweep(sweep_config, project="FLAVOR_UniMol_FL")
