"""
Uni-Mol LoRA Finetuning — BBBP | Focal Loss

Mirrors finetune_bbbp_fl.py from lora-finetuning-scripts/ but uses
...
Best model saved to:
  EffiChem_Extras/focal_BBBP/dptech__Uni__Mol_LoRA_Finetuned/
"""
...
SAVE_DIR    = EXTRAS_ROOT / "focal_loss_BBBP" / "dptech__Uni__Mol_LoRA_Finetuned"
...
logging.basicConfig(
    filename=str(LOG_DIR / "unimol_bbbp_fl.log"),
    level=logging.INFO,
...
# ── Sweep run ──────────────────────────────────────────────────────────────────
def run_training():
    run    = wandb.init(project="BBBP_UniMol_FL")
    config = run.config
...
if __name__ == "__main__":
    logging.info("=" * 60)
    logging.info("Uni-Mol LoRA Finetuning | BBBP | Focal Loss")
    logging.info("Additional modalities: 3D conformer (ETKDGv3) + Morgan ECFP4")
    logging.info("=" * 60)

    sweep_id = wandb.sweep(sweep_config, project="BBBP_UniMol_FL")
