"""
Uni-Mol LoRA Finetuning — BACE | Focal Loss

Mirrors finetune_bace_fl.py from lora-finetuning-scripts/ but uses
...
Best model saved to:
  EffiChem_Extras/focal_BACE/dptech__Uni__Mol_LoRA_Finetuned/
"""
...
SAVE_DIR    = EXTRAS_ROOT / "focal_loss_BACE" / "dptech__Uni__Mol_LoRA_Finetuned"
...
logging.basicConfig(
    filename=str(LOG_DIR / "unimol_bace_fl.log"),
    level=logging.INFO,
...
# ── Sweep run ──────────────────────────────────────────────────────────────────
def run_training():
    run    = wandb.init(project="BACE_UniMol_FL")
    config = run.config
...
if __name__ == "__main__":
    logging.info("=" * 60)
    logging.info("Uni-Mol LoRA Finetuning | BACE | Focal Loss")
    logging.info("Additional modalities: 3D conformer (ETKDGv3) + Morgan ECFP4")
    logging.info("=" * 60)

    sweep_id = wandb.sweep(sweep_config, project="BACE_UniMol_FL")
