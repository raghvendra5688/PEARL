"""
Uni-Mol LoRA Finetuning -- HALF_LIFE | HUBER Loss (regression)
...
Best model saved to:
  $PEARL_EXTRAS_V2/huber_loss_HALF_LIFE/dptech__Uni__Mol_LoRA_Finetuned/
"""
...
SAVE_DIR    = EXTRAS_ROOT / "huber_loss_HALF_LIFE" / "dptech__Uni__Mol_LoRA_Finetuned"
RESULTS_DIR = REPO_ROOT / "results" / "unimol_finetuning" / "half_life"
...
logging.basicConfig(
    filename=str(LOG_DIR / "unimol_half_life_huber.log"),
    level=logging.INFO,
...
# -- WandB sweep config ------------------------------------------------------------------
sweep_config = {
    "name":   "HALF_LIFE_HUBER_UniMol_Tuning",
    "method": "bayes",
...
# -- Sweep run ------------------------------------------------------------------------
def run_training():
    run    = wandb.init(project="HALF_LIFE_UniMol_HUBER")
    config = run.config
...
if __name__ == "__main__":
    logging.info("=" * 60)
    logging.info("Uni-Mol LoRA Finetuning | HALF_LIFE | HUBER Loss (regression)")
    logging.info("Additional modalities: 3D conformer (ETKDGv3) + Morgan ECFP4")
    logging.info("=" * 60)

    sweep_id = wandb.sweep(sweep_config, project="HALF_LIFE_UniMol_HUBER")
