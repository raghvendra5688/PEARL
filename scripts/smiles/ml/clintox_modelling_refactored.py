"""
ClinTox FT-Model-Embedding Modeling Pipeline (REFACTORED)
...
class Config:
    ...
    def __init__(self):
        ...
        # Data columns
        self.LABEL_COL = "FDA_APPROVED"
        self.SMILES_COL = "Standardized SMILES"

        # Columns to exclude (secondary labels that should NOT be used)
        self.EXCLUDE_COLS = ["CT_TOX"]  # Secondary toxicity label - excluded from modeling
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    log_file = log_dir / "clintox_ft_model_ml.log"
...
def extract_embeddings(df: pd.DataFrame, emb_col: str, label_col: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    IMPORTANT: Only the specified label_col is extracted. For ClinTox, this means
    only FDA_APPROVED is used as the target. Secondary labels like CT_TOX are
    present in the dataframe but are NOT extracted or used for modeling.

    Args:
        df: Input dataframe
        emb_col: Column name containing embeddings
        label_col: Column name containing labels (e.g., "FDA_APPROVED")
...
def load_split(split_name: str, base_path: Path, config: Config) -> pd.DataFrame:
    """
    Load data split with error handling and validation.

    Note: Secondary labels like CT_TOX are present in the CSV but are NOT used
    for modeling. Only FDA_APPROVED is used as the target label.
    """
    path = base_path / f"clintox_{split_name}_embed.csv"
    ...
    try:
        ...
        required_cols = [config.SMILES_COL, config.LABEL_COL]
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            raise ValueError(f"Missing required columns: {missing_cols}")

        # Log info about excluded columns
        excluded_present = [col for col in config.EXCLUDE_COLS if col in df.columns]
        if excluded_present:
            logging.info(f"Note: Secondary labels present but EXCLUDED from modeling: {excluded_present}")
            logging.info(f"Using only primary label: {config.LABEL_COL}")
...
def create_roc_curve(
    ...
    with plot_context():
        try:
            ...
            plt.title(f"ClinTox ROC Curves - {emb_name}")
...
def create_pr_curve(
    ...
    with plot_context():
        try:
            ...
            plt.title(f"ClinTox Precision-Recall Curves - {emb_name}")
            plt.legend(loc="best")
            plt.grid(alpha=0.3)
            plt.tight_layout()
...
def process_embedding(
    ...
    logger.info(f"Completed ClinTox modeling for embedding: {emb_name}")
...
def main():
    ...
    logger.info("="*60)
    logger.info("ClinTox FT-Model-Embedding Modeling Pipeline (REFACTORED)")
    logger.info("="*60)
    logger.info(f"Configuration: N_JOBS={config.N_JOBS}, OPTUNA_TRIALS={config.OPTUNA_TRIALS}")

    # Load data
    try:
        train_df = load_split("train", config.BASE_MODEL_ROOT, config)
        val_df = load_split("eval", config.BASE_MODEL_ROOT, config)
        test_df = load_split("test", config.BASE_MODEL_ROOT, config)
    except Exception as e:
    ...
    logger.info(
        f"ClinTox class distribution | 0: {n_neg}, 1: {n_pos}, "
    ...
    logger.info("All ClinTox base-model embedding experiments completed successfully.")
