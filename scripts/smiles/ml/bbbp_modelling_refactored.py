"""
BBBP FT-Model-Embedding Modeling Pipeline (REFACTORED)
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    log_file = log_dir / "bbbp_ft_model_ml.log"
...
def load_split(split_name: str, base_path: Path) -> pd.DataFrame:
    ...
    path = base_path / f"bbbp_{split_name}_embed.csv"
...
def process_embedding(
    ...
    logger.info(f"Completed BBBP modeling for embedding: {emb_name}")
...
def main():
    ...
    logger.info("="*60)
    logger.info("BBBP FT-Model-Embedding Modeling Pipeline (REFACTORED)")
    logger.info("="*60)
    ...
    logger.info(
        f"BBBP class distribution | 0: {n_neg}, 1: {n_pos}, "
    ...
    logger.info("All BBBP base-model embedding experiments completed successfully.")
