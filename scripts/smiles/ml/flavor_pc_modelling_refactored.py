"""
Flavor Embedding + PC Feature Modeling Pipeline (Multiclass) - Refactored

Multiclass classification for Flavor dataset:
- Target: Canonicalized Taste
...
- Increased Optuna trials from 30 to 30 (already optimal)
...
def setup_logging(config: Config) -> None:
    ...
    log_file = config.LOG_DIR / "flavor_pc_model_feature_ml.log"
...
def build_feature_matrix(
    ...
    # Parse embeddings (NOT sanitized)
    emb = parse_embedding_column(df[emb_col])

    # Extract and sanitize chemical features only
    ...
    y = label_encoder.transform(df[config.LABEL_COL])
...
def load_split(config: Config, split_name: str) -> pd.DataFrame:
    ...
    try:
        path = config.DATA_ROOT / f"fart_{split_name}_features.csv"
...
def run_optimization(
    ...
    study.optimize(
        lambda t: optimize_model(t, model_type, X_train, y_train, X_val, y_val, sample_weights, n_classes, config),
...
def train_and_evaluate(
    ...
    try:
        ...
        metrics = {
            ...
            "AUC": round(roc_auc_score(y_test, y_prob, multi_class="ovr"), 3)
        }
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("Starting Flavor Embedding + PC Feature Modeling Pipeline (Multiclass)")
    ...
    try:
        ...
        for emb_name in config.EMBED_COLS:
            ...
            try:
                ...
                X_train, y_train = build_feature_matrix(train_df, emb_name, label_encoder, config)
                X_val, y_val = build_feature_matrix(val_df, emb_name, label_encoder, config)
                X_test, y_test = build_feature_matrix(test_df, emb_name, label_encoder, config)
                ...
                best_params = {
                    "XGBoost": run_optimization("xgb", X_train, y_train, X_val, y_val, sample_weights, n_classes, config),
                    "LightGBM": run_optimization("lgb", X_train, y_train, X_val, y_val, sample_weights, n_classes, config),
                    "CatBoost": run_optimization("cb", X_train, y_train, X_val, y_val, sample_weights, n_classes, config)
                }
        ...
        logging.info("=" * 80)
        logging.info("Flavor FT model with feature modeling completed successfully")
        logging.info("=" * 80)
