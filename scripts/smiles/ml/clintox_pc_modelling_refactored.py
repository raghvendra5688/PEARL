"""
ClinTox Embedding + PC Feature Modeling Pipeline - Refactored

Binary classification for ClinTox (Clinical Toxicity):
- Target: FDA_APPROVED
...
class Config:
    ...
    def __init__(self):
        ...
        self.DATA_ROOT = self._validate_path(
            Path(os.getenv('CLINTOX_PC_DATA_ROOT', self.BASE_DIR / "data" / "finetuned_pc_embeddings" / "clintox_Embeddings"))
        )
        self.OUTPUT_ROOT = self._validate_path(
            Path(os.getenv('CLINTOX_PC_OUTPUT_ROOT', self.BASE_DIR / "results" / "ft_embeddings" / "clintox_PC_FT_Results")),
            create=True
        )
...
def setup_logging(config: Config) -> None:
    ...
    log_file = config.LOG_DIR / "clintox_ft_pc_model_feature_ml.log"
...
def load_split(config: Config, split_name: str) -> pd.DataFrame:
    ...
    try:
        path = config.DATA_ROOT / f"clintox_{split_name}_features.csv"
...
def train_and_evaluate(
    ...
    try:
        ...
        logging.info(f"[ClinTox | {emb_name}] {name} metrics: {metrics}")
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("Starting ClinTox Embedding + PC Feature Modeling Pipeline")
    ...
    try:
        ...
        logging.info("=" * 80)
        logging.info("ClinTox TASK: FDA_Approved")
        logging.info("=" * 80)
        ...
        logging.info("=" * 80)
        logging.info("ClinTox FT model with features modeling completed successfully")
        logging.info("=" * 80)
