"""
RAG-Augmented Modelling (HF) — herg/dili/caco2/half_life

Phase 8 of the editor-response revision (see manuscript/editor_response_suggestions.md):
trains XGBoost/LightGBM/CatBoost on [finetuned-CLM embedding + PC features +
ZINC-250k RAFE features] for both classification (herg/dili) and regression
(caco2/half_life), for the HF modality.

Structurally this is ft_modelling_new_datasets.py (same feature-matrix builder,
Optuna search via pc_only_modelling.run_optimization/build_model, target
transform handling, bootstrap CI) plus one extra step: merging in the
RAFE feature CSV produced by rag_feature_extraction_new_datasets.py before
building X, following the [emb | PC | RAG] concatenation convention from
rag_modelling_bace.py (the original 4-dataset RAG modelling script).

Usage:
    python rag_modelling_new_datasets.py --dataset herg
    python rag_modelling_new_datasets.py --dataset all --trials 20

Input:
    $PEARL_EXTRAS_V2/finetuned_pc_embeddings/{Dataset}_Embeddings/{prefix}_{split}_features.csv
    data/rag_features/{dataset}/{col_name}_{split}_rag.csv

Output:
    results/rag/{DATASET}/{col_name}/ — metrics, models
"""
...
PC_ROOT = EXTRAS_ROOT / "finetuned_pc_embeddings"
RAG_ROOT = REPO_ROOT / "data" / "rag_features"
...
def build_feature_matrix(
    ...
    """Merge [embedding + PC] df with the RAG-feature df on SMILES, build
    [emb | PC | RAG]. Mirrors rag_modelling_bace.py's build_feature_matrix()."""
    ...
    for embed_col in all_embed_cols:
        ...
        if train_rag is None or test_rag is None:
            logging.warning(f"[{dataset_key} | {col_name}] RAG features missing under {rag_dir}, "
                            "run rag_feature_extraction_new_datasets.py first, skipping")
            continue

        out_dir = out_root / col_name
        setup_logging(out_dir / "logs", f"rag_{dataset_key}_{col_name}")
        logging.info("=" * 60)
        logging.info(f"RAG Modelling (HF) | dataset={dataset_key} embed={col_name}")
        logging.info("=" * 60)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="RAG-augmented HF modelling for herg/dili/caco2/half_life")
