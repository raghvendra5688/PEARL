"""
Full Feature Extraction Pipeline - Refactored

This script extracts chemical and molecular features:
...
class Config:
    ...
    def __init__(self):
        ...
        self.DATASET_CONFIG = {
            "BBBP_Embeddings": ["p_np"],
            "clintox_Embeddings": ["FDA_APPROVED"],
            "flavor_Embeddings": ["Canonicalized Taste"],
        }

        self.dataset_mapping = {
            "BBBP_Embeddings": "bbbp",
            "clintox_Embeddings": "clintox",
            "flavor_Embeddings": "fart"
        }

        self.smiles_mapping = {
            "BBBP_Embeddings": "Standardized SMILES",
            "clintox_Embeddings": "smiles",
            "flavor_Embeddings": "Standardized SMILES"
        }
...
def setup_logging(config: Config) -> None:
    ...
    log_file = config.LOG_DIR / "ft_model_feature_extraction.log"
...
def extract_features(df: pd.DataFrame, label_cols: List[str], config: Config) -> pd.DataFrame:
    """
    ...
    mols = df["Standardized SMILES"].apply(safe_mol_from_smiles)
    ...
    embed_cols = [c for c in df.columns if c not in label_cols + ["Standardized SMILES"]]
...
def process_dataset(
    ...
    for split in config.SPLITS:
        ...
        try:
            ...
            # Extract features
            feature_df = extract_features(df, label_cols, config)
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("Starting Full Feature Extraction Pipeline")
    ...
    try:
        ...
        logging.info("=" * 80)
        logging.info("Full feature extraction pipeline finished successfully")
        logging.info("=" * 80)
