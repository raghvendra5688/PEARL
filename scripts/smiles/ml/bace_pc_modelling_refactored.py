"""
BACE Embedding + PC Feature Modeling Pipeline - Refactored

Binary classification for BACE (Beta-secretase 1):
- Target: Class
...
class Config:
    ...
    def __init__(self):
        ...
        self.DATA_ROOT = self._validate_path(
            Path(os.getenv('BACE_PC_DATA_ROOT', self.BASE_DIR / "data" / "finetuned_pc_embeddings" / "BACE_Embeddings"))
        )
        self.OUTPUT_ROOT = self._validate_path(
            Path(os.getenv('BACE_PC_OUTPUT_ROOT', self.BASE_DIR / "results" / "ft_embeddings" / "BACE_PC_FT_Results")),
            create=True
        )
...
def setup_logging(config: Config) -> None:
    ...
    log_file = config.LOG_DIR / "bace_ft_pc_model_feature_ml.log"
...
def load_split(config: Config, split_name: str) -> pd.DataFrame:
    ...
    try:
        path = config.DATA_ROOT / f"bace_{split_name}_features.csv"
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("Starting BACE Embedding + PC Feature Modeling Pipeline")
    ...
    try:
        ...
        logging.info("=" * 80)
        logging.info("BACE FT model with PC features modeling completed successfully")
        logging.info("=" * 80)
