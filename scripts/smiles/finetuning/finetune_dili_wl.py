...
RESULTS_DIR = REPO_ROOT / "results" / "lora_finetuning" / "dili"
...
TRAINING_DIR = PEARL_EXTRAS / "lora_training" / "dili"  # scratch per-trial checkpoints, not repo-tracked
...
def data_load():
    train_dili=pd.read_csv(str(DATA_DIR / 'train_clean.csv'))
    val_dili=pd.read_csv(str(DATA_DIR / 'valid_clean.csv'))

    return train_dili, val_dili
...
def data_prep(data_process,tokenizer):
    ...
    labels = data_process['DILI_Label'].astype(int).tolist()
...
sweep_config = {
"name": "DILI_Hyperparameter_Tuning",
"method": "bayes",
"metric": {
    "goal": "maximize",
...
for key in MODEL_NAME_MAP.keys():
    ...
    test_labels = test_data['DILI_Label'].astype(int).tolist()
    ...
    save_path = str(PEARL_EXTRAS / "weighted_loss_DILI" / (key+"_LoRA_Finetuned")) + "/"
...
import shutil
shutil.rmtree(str(TRAINING_DIR / "WL_Loss"), ignore_errors=True)
