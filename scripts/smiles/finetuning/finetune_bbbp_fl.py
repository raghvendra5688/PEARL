...
RESULTS_DIR = REPO_ROOT / "results" / "lora_finetuning" / "bbbp"
MODELS_DIR = REPO_ROOT / "models" / "lora_finetuned" / "bbbp"
TRAINING_DIR = REPO_ROOT / "models" / "lora_training" / "bbbp"
...
def data_load():
    train_bbbp=pd.read_csv(str(DATA_DIR / 'train_clean.csv'))
    val_bbbp=pd.read_csv(str(DATA_DIR / 'valid_clean.csv'))
...
for key in MODEL_NAME_MAP.keys():
    ...
    save_path = str(MODELS_DIR / "focal_loss" / (key+"_LoRA_Finetuned")) + "/"
