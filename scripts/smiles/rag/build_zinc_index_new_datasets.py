"""
Build FAISS-GPU Indices for HF ZINC-250k Embeddings — herg/dili/caco2/half_life

Mirrors build_zinc_index.py for the 4 new TDC datasets. Reads the .npy files
produced by embed_zinc250k_new_datasets.py, L2-normalises them (cosine
...
Usage:
    python build_zinc_index_new_datasets.py --dataset herg
    python build_zinc_index_new_datasets.py --dataset all
...
# col_names produced by embed_zinc250k_new_datasets.py per dataset
DATASET_COL_NAMES = {
    "herg":      ["ChemBERTa_77M_MTR_FL", "ChemBERTa_77M_MLM_FL", "MolFormer_Finetuned_FL",
                  "ChemBERTa_77M_MTR_WL", "ChemBERTa_77M_MLM_WL", "MolFormer_Finetuned_WL"],
    "dili":      ["ChemBERTa_77M_MTR_FL", "ChemBERTa_77M_MLM_FL", "MolFormer_Finetuned_FL",
                  "ChemBERTa_77M_MTR_WL", "ChemBERTa_77M_MLM_WL", "MolFormer_Finetuned_WL"],
    "caco2":     ["ChemBERTa_77M_MTR_Huber", "ChemBERTa_77M_MLM_Huber", "MolFormer_Finetuned_Huber"],
    "half_life": ["ChemBERTa_77M_MTR_Huber", "ChemBERTa_77M_MLM_Huber", "MolFormer_Finetuned_Huber"],
}
...
def setup_logging(dataset: str) -> None:
    ...
    logging.basicConfig(
        filename=str(LOG_DIR / f"build_zinc_index_new_datasets_{dataset}.log"),
        level=logging.INFO,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build FAISS indices for new-dataset HF ZINC-250k embeddings.")
    ...
    for dataset in datasets:
        ...
        logging.info("=" * 60)
        logging.info(f"Build HF ZINC FAISS Index | dataset={dataset} | GPU={args.gpu_id}")
        ...
        for col_name in DATASET_COL_NAMES[dataset]:
            ...
            if not npy_path.exists():
                logging.warning(f"Embedding file not found, skipping: {npy_path}. Run embed_zinc250k_new_datasets.py first.")
                continue
        ...
        logging.info(f"\nDone. All HF FAISS indices for {dataset} saved to: {index_dir}")
