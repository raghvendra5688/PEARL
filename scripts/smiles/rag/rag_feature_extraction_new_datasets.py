"""
RAG Feature Extraction (HF) — herg/dili/caco2/half_life

Mirrors rag_feature_extraction.py for the 4 new TDC datasets. For a given
dataset, queries each task-specific ZINC-250k FAISS index (built by
build_zinc_index_new_datasets.py) with every molecule in the train/eval/test
splits and computes the same chemical-space context features (57 per model):
...
herg/dili use 6 models (3 base x FL/WL); caco2/half_life use 3 (3 base x Huber).

PCA models are saved to $PEARL_EXTRAS_V2/rag_pca/{dataset}/{col_name}_pca.pkl
and must be fit on train before eval/test -- splits are processed in order
train -> eval -> test automatically.

Usage:
    python rag_feature_extraction_new_datasets.py --dataset herg

Input:
    $PEARL_EXTRAS_V2/finetuned_embeddings/{Dataset}_Embeddings/{prefix}_{split}_embed.csv
    $PEARL_EXTRAS_V2/rag_indices/{dataset}/{col_name}.index
    $PEARL_EXTRAS_V2/rag_indices/{dataset}/meta.pkl

Output:
    data/rag_features/{dataset}/{col_name}_{split}_rag.csv
    $PEARL_EXTRAS_V2/rag_pca/{dataset}/{col_name}_pca.pkl
"""
...
FT_EMBED_ROOT = EXTRAS_ROOT / "finetuned_embeddings"
...
# dataset -> list of {col_name, csv_col} (col_name matches the .npy/.index name
# produced by embed_zinc250k_new_datasets.py / build_zinc_index_new_datasets.py)
MODEL_REGISTRY = {
    "herg": [
    ...
    "caco2": [
        {"col_name": "ChemBERTa_77M_MTR_Huber",   "csv_col": "ChemBERTa_77M_MTR_Huber_embeddings"},
        {"col_name": "ChemBERTa_77M_MLM_Huber",   "csv_col": "ChemBERTa_77M_MLM_Huber_embeddings"},
        {"col_name": "MolFormer_Finetuned_Huber", "csv_col": "MolFormer_Finetuned_Huber_embeddings"},
    ],
}
...
def setup_logging(dataset: str) -> None:
    ...
    logging.basicConfig(
        filename=str(LOG_DIR / f"rag_feature_extraction_new_datasets_{dataset}.log"),
        level=logging.INFO,
...
# ── Embedding parsing (matches finetuned_model_embeddings_new_datasets.py output) ──
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract new-dataset ZINC-250k HF RAG features.")
    ...
    logging.info("=" * 60)
    logging.info(f"RAG Feature Extraction (new datasets) | dataset={dataset} | GPU={args.gpu_id}")
    logging.info("=" * 60)
    ...
    if not meta_path.exists():
        raise FileNotFoundError(f"meta.pkl not found: {meta_path}. Run build_zinc_index_new_datasets.py first.")
    ...
    for model_info in MODEL_REGISTRY[dataset]:
        ...
        if not index_path.exists():
            logging.warning(f"Index not found, skipping: {index_path}. Run build_zinc_index_new_datasets.py first.")
            continue
    ...
    logging.info("\nHF RAG feature extraction (new datasets) complete.")
