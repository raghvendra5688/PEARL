"""
RAG-Augmented Modelling Pipeline — BACE

Binary classification (target: Class) using:
    [Finetuned CLM embedding] + [PC features] + [ZINC-250k RAG features]

For each of 6 finetuned model variants, merges RAG features onto the existing
finetuned_pc_embeddings CSV, then runs Optuna-tuned XGBoost / LightGBM / CatBoost
(same hyperparameter space and CV strategy as the rest of EffiChem-2.0).

Input:
    data/finetuned_pc_embeddings/BACE_Embeddings/bace_{split}_features.csv
    data/rag_features/bace/{col_name}_{split}_rag.csv

Output:
    results/rag/bace/{col_name}/ — metrics, models, ROC/PR curves
"""
...
class Config:
    BASE_DIR      = REPO_ROOT
    DATA_ROOT     = EXTRAS_ROOT / "PC_FT_All_Embeddings" / "BACE_Embeddings"
    RAG_ROOT      = REPO_ROOT / "data" / "rag_features" / "bace"
    OUTPUT_ROOT   = REPO_ROOT / "results" / "rag" / "bace"
    ...
    # Maps embedding CSV column → RAG feature file col_name prefix
    EMBED_TO_RAG = {
...
def setup_logging(config: Config) -> None:
    ...
    fh  = logging.FileHandler(config.LOG_DIR / "rag_bace.log", mode="a")
    fh.setFormatter(fmt)
...
# ── Data loading ──────────────────────────────────────────────────────────────
def load_split(config: Config, split: str) -> pd.DataFrame:
    path = config.DATA_ROOT / f"{config.FILE_PREFIX}_{split}_features.csv"
    if not path.exists():
        raise FileNotFoundError(f"PC+embedding CSV not found: {path}")
...
def build_feature_matrix(
    ...
    """Merge PC+embedding df with RAG df on SMILES, build [emb | PC | RAG] matrix."""
    merged = df_pc.merge(
        df_rag.drop(columns=[c for c in df_rag.columns if c == config.LABEL_COL], errors="ignore"),
        on=config.SMILES_COL, how="inner",
    )

    emb = parse_embedding_col(merged[emb_col])

    # PC features: everything that isn't meta/embed/label
    skip = [config.SMILES_COL, config.LABEL_COL] + config.EMBED_COLS
    ...
    with _plot_ctx():
        ...
        for name, prob in preds.items():
            ...
            axes[1].plot(rec, prec, label=f"{name} (AP={ap:.3f})",
                         color=config.MODEL_COLORS[name], linewidth=2)
        axes[0].plot([0,1],[0,1],"k--"); axes[0].set_title(f"BACE ROC — {tag}")
...
def run_for_embedding(emb_col, train_df, val_df, test_df, config):
    ...
    try:
        X_tr, y_tr = build_feature_matrix(train_df, rag_train, emb_col, config)
        X_va, y_va = build_feature_matrix(val_df,   rag_val,   emb_col, config)
        X_te, y_te = build_feature_matrix(test_df,  rag_test,  emb_col, config)
...
def main():
    ...
    logging.info("=" * 70)
    logging.info("RAG-Augmented BACE Modelling (emb + PC + ZINC RAG features)")
    ...
    for emb_col in config.EMBED_COLS:
        try:
            run_for_embedding(emb_col, train_df, val_df, test_df, config)
        except Exception as e:
            logging.error(f"Failed for {emb_col}: {e}", exc_info=True)

    logging.info("\nBACE RAG modelling complete.")
