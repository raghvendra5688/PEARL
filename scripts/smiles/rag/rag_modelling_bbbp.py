"""
RAG-Augmented Modelling Pipeline — BBBP

Binary classification (target: p_np) using:
    [Finetuned CLM embedding] + [PC features] + [ZINC-250k RAG features]

Input:
    data/finetuned_pc_embeddings/BBBP_Embeddings/bbbp_{split}_features.csv
    data/rag_features/bbbp/{col_name}_{split}_rag.csv

Output:
    results/rag/bbbp/{col_name}/ — metrics, models, ROC/PR curves
"""
...
class Config:
    BASE_DIR      = REPO_ROOT
    DATA_ROOT     = EXTRAS_ROOT / "PC_FT_All_Embeddings" / "BBBP_Embeddings"
    RAG_ROOT      = REPO_ROOT / "data" / "rag_features" / "bbbp"
    OUTPUT_ROOT   = REPO_ROOT / "results" / "rag" / "bbbp"
...
def setup_logging(config: Config) -> None:
    ...
    fh  = logging.FileHandler(config.LOG_DIR / "rag_bbbp.log", mode="a"); fh.setFormatter(fmt)
...
def load_rag_features(config: Config, col_name: str, split: str) -> Optional[pd.DataFrame]:
    ...
    if not path.exists():
        logging.warning(f"RAG features missing: {path}")
        return None
    return pd.read_csv(str(path))


def build_feature_matrix(df_pc, df_rag, emb_col, config) -> Tuple[np.ndarray, np.ndarray]:
    merged = df_pc.merge(
...
def run_optimisation(model_type, X, y, spw, config):
    ...
    study.optimize(lambda t: _objective(t, model_type, X, y, spw, config), n_trials=config.OPTUNA_TRIALS)
...
def run_for_embedding(emb_col, train_df, val_df, test_df, config):
    ...
    logging.info(f"\n{'='*20} {col_name} {'='*20}")
    rag_tr = load_rag_features(config, col_name, "train")
    rag_va = load_rag_features(config, col_name, "eval")
    rag_te = load_rag_features(config, col_name, "test")
    if rag_tr is None or rag_te is None:
        logging.warning(f"  Skipping {col_name} — RAG features missing.")
        return
    try:
        X_tr, y_tr = build_feature_matrix(train_df, rag_tr, emb_col, config)
        X_va, y_va = build_feature_matrix(val_df,   rag_va, emb_col, config)
        X_te, y_te = build_feature_matrix(test_df,  rag_te, emb_col, config)
    ...
    preds["XGBoost"] = train_and_evaluate(
        "XGBoost", xgb.XGBClassifier(objective="binary:logistic", eval_metric="logloss",
            scale_pos_weight=spw, random_state=config.RANDOM_SEED, tree_method="hist",
            n_jobs=config.N_JOBS, **best["XGBoost"]),
        X_tr, y_tr, X_te, y_te, out_dirs, col_name, config)
    preds["LightGBM"] = train_and_evaluate(
        "LightGBM", lgb.LGBMClassifier(class_weight="balanced", random_state=config.RANDOM_SEED,
    ...
    preds["CatBoost"] = train_and_evaluate(
        "CatBoost", cb.CatBoostClassifier(auto_class_weights="Balanced", loss_function="Logloss",
            random_seed=config.RANDOM_SEED, verbose=0, thread_count=config.N_JOBS, **best["CatBoost"]),
        X_tr, y_tr, X_te, y_te, out_dirs, col_name, config)
...
def main():
    ...
    logging.info("=" * 70)
    logging.info("RAG-Augmented BBBP Modelling (emb + PC + ZINC RAG features)")
